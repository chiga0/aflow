...
发布前可手工触发同一条真实仓库门禁。它会创建一个初始测试失败的 Git
仓库，要求 Qwen 修改实现和文档，并校验隔离 worktree、实时事件、完整测试、
干净 patch、`aflow/*` commit、audit bundle 以及源检出目录不变：

```bash
gh workflow run deploy-runtime.yml --ref main \
  -f executor_strategy=shared \
  -f qwen_validate_repository=true \
  -f qwen_repository_timeout=900
```

只有输出 `execution_mode=real-cli`、`test_passed=true`，且 `changed_files`
不含缓存或字节码文件时，真实仓库门禁才通过。

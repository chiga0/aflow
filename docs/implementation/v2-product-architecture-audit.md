...
## 2026-07-06 P0：运行详情状态与继续对话事故复盘

### 现象

线上运行详情页暴露了三个严重可用性问题：

- 用户已审批且任务已完成后，右下角活跃运行 Dock 仍显示 `待审批` 和 `running`。
- Run Detail 的任务进度优先展示历史 permission request，覆盖了真实完成状态。
- “继续对话”输入区在宽屏右侧审计栏场景下可被挤出可视区域，且 Enter 没有提交。

### 根因

| 问题 | 根因 |
| --- | --- |
| 待审批残留 | Run Detail 和 Dock 各自解析 permission 状态，未统一处理 `permission_id` / `requestId` / qwen raw 嵌套形态 |
| completed 被覆盖 | `runTaskProgress` 先判断 pending permission，再判断 terminal status，导致历史事件覆盖终态 |
| Dock 显示 running | Dock 只信任 runs 列表 status，未用事件流中的 `run.completed` 推导有效终态 |
| 输入不可用 | 运行详情页左列缺少 `min-w-0`，输入区按钮在侧栏布局下可被挤压；Textarea 没有 Enter 提交处理 |
| 测试漏网 | 既有测试只覆盖“出现审批卡片并点击批准”的 happy path，没有覆盖“审批提交后/完成后所有组件状态一致”的回归场景 |

### 修复要求

| 要求 | 验收 |
| --- | --- |
| permission id 统一归一化 | `permission_id`、`permissionId`、`request_id`、`requestId`、`raw.data.requestId` 均可匹配 |
| 终态优先 | `run.completed` / `run.failed` / `run.cancelled` 必须覆盖历史 pending permission |
| Dock 状态一致 | Dock 不能在事件流已有终态时继续显示 `running` 或 `待审批` |
| 继续对话可操作 | 桌面和移动端都必须看到 Send 按钮；Enter 发送，Shift+Enter 换行 |
| E2E 守护 | 页面级 E2E 必须等待真实 `/session/{run_id}/prompt` POST，不能只断言按钮存在 |

### 后续防复发

- 任何改动 Run Detail、Active Runs Dock、permission projection、daemon event projection 的 PR，必须跑 `npm run test` 和 `npm run test:e2e`。
- 新增运行状态时必须同时更新 `pendingPermissionRequests`、Dock 状态推导、TaskProgress 终态优先级测试。
- 用户端状态展示不得只依赖单一投影字段；如果 runs 列表滞后，应以 canonical events 的终态作为显示修正。
- 多 Agent / Mission 页面必须补充“多个子任务、多个 profile、多个 run_id”的 E2E，避免用户误以为多 Agent 编排实际只启动单 Agent。

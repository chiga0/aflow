...
aflow 的协议分层目标是：内部执行可控，外部互操作开放，工具接入不和 Agent 通信混在一起。
...
内部执行不要直接暴露成 A2A。A2A 不关心 workspace、权限审批、执行单元资源、CLI 日志和 artifact 生命周期；这些仍应由 aflow 控制面管理。

## 2. 内部 adapter contract

所有真实 Agent CLI 进入 aflow 时，应被转换为统一事件：
...
aflow 可以在 adapter 层兼容 ACP transport，但控制面的权限、审计、artifact、retry/replay 仍由 aflow 负责。

...
#
#   deploy/deploy_vps.sh user@host [/opt/aflow-lite]
#
...
# QWEN_SETTINGS_FILE or by placing deploy/qwen-settings.json (git-ignored).
...
if [ -f "$QWEN_SETTINGS" ]; then
  ...
  echo "   place one at deploy/qwen-settings.json or set QWEN_SETTINGS_FILE."
fi

...
AUTH_EMAIL="${AUTH_EMAIL:-${RUNTIME_AUTH_EMAIL:-}}"
AUTH_PASSWORD="${AUTH_PASSWORD:-${RUNTIME_AUTH_PASSWORD:-}}"
AUTH_NAME="${AUTH_NAME:-Cloud Agents Owner}"
...
QWEN_CONTAINER_BUILD_NORMALIZED="$(
  printf '%s' "$QWEN_CONTAINER_BUILD" | tr '[:upper:]' '[:lower:]'
)"
case "$QWEN_CONTAINER_BUILD_NORMALIZED" in
  1 | true | yes)
...
REMOTE_ENV=(
  ...
  "AUTH_EMAIL=$(shell_quote "$AUTH_EMAIL")"
  "AUTH_PASSWORD=$(shell_quote "$AUTH_PASSWORD")"
  "AUTH_NAME=$(shell_quote "$AUTH_NAME")"
...
if [[ -z "$AUTH_EMAIL" ]]; then
  if [[ "$BASIC_AUTH_USER" == *@* ]]; then
    AUTH_EMAIL="$BASIC_AUTH_USER"
  else
    AUTH_EMAIL="cloudagents@local.test"
  fi
fi
...
RUN_MANAGER_BOOTSTRAP_EMAIL=$AUTH_EMAIL
RUN_MANAGER_BOOTSTRAP_PASSWORD=$AUTH_PASSWORD
RUN_MANAGER_BOOTSTRAP_NAME=$AUTH_NAME
...
server {
    ...
    location /agentflow/ {
        ...
        proxy_set_header X-Forwarded-Prefix /agentflow;
    ...
    location /agentflow-worker/ {
        ...
        proxy_read_timeout 3600s;
        proxy_send_timeout 3600s;
        rewrite ^/agentflow-worker/(.*)\$ /\$1 break;
        proxy_pass http://127.0.0.1:8765;
    }

...
def main() -> int:
    ...
    if args.command == "up":
        ...
        try:
            ...
            print("runtime startup diagnostics:", file=sys.stderr)
            compose_diagnostics(env_file)
            raise
...
def init_environment(path: Path, *, bind: str) -> dict[str, str]:
    if path.exists():
        ...
        missing_identity = {
            key: value for key, value in identity.items() if not env.get(key)
        }
        if missing_identity:
            with path.open("a", encoding="utf-8") as handle:
                handle.write(render_env(missing_identity, include_header=False))
            env.update(missing_identity)
        try:
            path.chmod(0o600)
...
def render_env(values: dict[str, str], *, include_header: bool = True) -> str:
    lines = []
    if include_header:
        lines.append("# Generated by scripts/local_stack.py. Do not commit this file.")
    lines.extend(f"{key}={value}" for key, value in values.items())
    return "\n".join(lines) + "\n"


def host_runtime_identity() -> dict[str, str]:
    return {
        "RUNTIME_UID": str(getattr(os, "getuid", lambda: 1000)()),
        "RUNTIME_GID": str(getattr(os, "getgid", lambda: 1000)()),
    }
...
def compose_diagnostics(env_file: Path) -> None:
    for arguments in (("ps", "--all"), ("logs", "--tail", "200", "runtime")):
        try:
            compose(env_file, *arguments)
